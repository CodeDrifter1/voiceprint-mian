...
class VoiceprintDB:
    ...
    def list_voiceprints(self) -> List[Dict]:
        ...
        Returns:
            List[Dict]: [{"speaker_id": str, "created_at": str}, ...]
        """
        try:
            with db_connection.get_cursor() as cursor:
                sql = "SELECT speaker_id, created_at FROM voiceprints ORDER BY created_at DESC"
                cursor.execute(sql)
                results = cursor.fetchall()
                return [
                    {"speaker_id": row[0], "created_at": str(row[1]) if row[1] else None}
                    for row in results
                ]
        except Exception as e:
